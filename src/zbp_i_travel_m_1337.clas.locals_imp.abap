...
CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    ...
    TYPES tt_travel_update TYPE TABLE FOR UPDATE zi_travel_m_1337.
    ...
    METHODS set_status_canceled        FOR MODIFY IMPORTING   keys FOR ACTION travel~cancelTravel              RESULT result.
...
********************************************************************************
  METHOD get_features.
    ...
    READ ENTITY IN LOCAL MODE zi_travel_m_1337 FROM VALUE #( FOR keyval IN keys
                                                      (  %key                    = keyval-%key
    ...
    result = VALUE #( FOR ls_travel IN lt_travel_result
                      ( %key                           = ls_travel-%key
                        ...
                        %features-%action-cancelTravel = COND #( WHEN ls_travel-overall_status = 'X'
                                                                    THEN if_abap_behv=>fc-o-disabled ELSE if_abap_behv=>fc-o-enabled   )
                      )
                      ).
  ...
  METHOD validate_agency.
    ...
    READ ENTITY IN LOCAL MODE zi_travel_m_1337\\travel FROM VALUE #(
  ...
  METHOD calculatetravelkey.
    READ ENTITIES OF zi_travel_m_1337 IN LOCAL MODE
        ENTITY Travel
          FIELDS ( travel_id )
    ...
    "Get max travelID
    SELECT SINGLE FROM ztravel_1337 FIELDS MAX( travel_id ) INTO @DATA(lv_max_travelid).

    "update involved instances
    MODIFY ENTITIES OF zi_travel_m_1337 IN LOCAL MODE
      ENTITY Travel
